...
//            log.error("[Minio工具类]>>>> 判断文件夹是否存在，异常：", e);
            System.out.println("[Minio工具类]>>>> 判断文件夹是否存在，异常：");
            e.printStackTrace();
            exist = false;
        }
        return exist;
     ...
     */
    @SneakyThrows(Exception.class)
    public ObjectWriteResponse uploadFile2(String bucketName, InputStream inputStream, String objectName, String contentType) throws Exception {
        return minioClient.putObject(
                PutObjectArgs.builder()
                        .bucket(bucketName)
                        .object(objectName)
     ...
     * @return
     */
    public void removeFiles(String bucketName, List<String> keys) throws Exception {
...
//                log.error("[Minio工具类]>>>> 批量删除文件，异常：", e);
                System.out.println("[Minio工具类]>>>> 批量删除文件，异常：");
                e.printStackTrace();
            }
        });
    }
